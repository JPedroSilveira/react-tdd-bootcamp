...
const GuessedWords: React.FC<GuessedWordsProps> = ({ words }) => {
	const hasGuessedWords = words.length > 0
		...
		return (
			<div data-test='guessed-words-guessed-section'>
				...
				<table>
					...
					<tbody>{renderGuessedWordsRows()}</tbody>
				</table>
			</div>
		)
	}
	...
	return (
		<div className='guessed-words' data-test='guessed-words'>
			{hasGuessedWords ? renderGuessedSection() : renderInstruction()}
		</div>
	)
}
